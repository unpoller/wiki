...
1.  leave general settings alone - container name should be golift-unifi-poller1 unless you created other iunifi-pollers
1.  Click advanced settings
1.  on the network tab
    *   add your network, in this example, Grafana_Net
    *   remove the default bridge (usually called bridge)
...
1.  You will need to CD to the root docker directory you created earlier (in this example the /docker folder containing the `/grafana` folder.
1.  The command format is cd `/volume{x}/{dirname}` on my system this shared folder is on volume 3 so for me it is: `cd /volume3/docker`
